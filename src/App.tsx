...
const App = () => {
  ...
  if (gameState === 'start') return <StartScreen onStart={startGame} />;
  if (gameState === 'finished') return <FeedbackScreen answers={userAnswers} onRestart={restartGame} />;
  if (!questions.length) return <div className="flex items-center justify-center h-screen">Error: No questions loaded. Check the server at http://localhost:3000/db.json. See console for details.</div>;
